...
import com.park.pda.action.v1.finger.param.Param_finger_userOrderList;
import com.park.pda.service.Finger_userOrderListBiz;
...
public class Read_Finger_userOrderListAction extends BaseV1Controller {

  private static final long serialVersionUID = -3599663972360625463L;
  @Autowired
  protected Finger_userOrderListBiz userOrderListBiz;
  @Autowired(required = false)
  private HttpServletRequest request;

  @RequestMapping(value = "/finger_user_orderList")
  public String getUserOrderList(HttpServletRequest request, HttpServletResponse response,Param_finger_userOrderList param) {
    ...
    try {
      ...
      userOrderListBiz.getUserOrderList(returnData, param.ui_tel,param.page,param.size);
    } catch (Exception e) {
      log.error("Read_Finger_userOrderListAction getUserOrderList is error (DEVICE-JAVA)- P", e);
   ...
   * @throws Exception
   */
  private boolean checkParam(Param_finger_userOrderList param, ReturnDataNew returnData) throws Exception{
    //参数检查
    if (param == null) {
      //参数传递错误
